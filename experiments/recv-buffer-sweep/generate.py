...
def arms(phase: str):
    ...
    modes = ["echo", "forward"]
    conns = [64] if phase == "a" else [64, 512]
...
def render_chunk(chunk, idx, bins_artifact, phase, warmup, duration):
...
name = "ringline recv-buffer sweep phase-{phase} chunk-{idx}"
    ...
    for a in chunk:
        ...
        if a["mode"] == "forward":
            ...
            client_cmd = (
                ...
                f"--clients {a['conns']} --threads 16 --msg-size 16384 "
            ...
            client_cmd = (
                ...
                f"--clients {a['conns']} --threads 16 --msg-size {a['msg']} "
                "--warmup {warmup} --duration {duration}"
            )
        server_steps.append(
...
def main():
    ...
    for i, chunk in enumerate(chunks, 1):
        ...
        path.write_text(
            render_chunk(chunk, i, args.bins_artifact, args.phase, args.warmup, args.duration)
        )
