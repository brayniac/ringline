...
| [2026-09 — Direct-forward path](2026-09-direct-forward-path.md) | Sep 2026 · open · follows #415, #416 | **Intent only.** Mode A wakes a task per provided buffer; `run_direct_echo` submits from the CQE handler and measures 0.985 instr/byte against forwarding's 1.26 at the same 16 KiB buffer. The #416 forwarding gap looks like a missing fast path, not a wrong default |
