...
| [2026-09 — Incremental provided-buffer consumption](2026-09-incremental-buffer-consumption.md) | Sep 2026 · open · follows #415 (282773b), blocks on #416 | **Intent only.** `IOU_PBUF_RING_INC` (Linux 6.12) to break the payload-per-completion vs ring-depth tie. Motivated by a measured ~6,600 instructions per completion and a 4-deep ring starving 3M times at the size that is otherwise fastest. A remembered prior negative result is recorded as uncheckable |
| [2026-09 — Direct-forward path](2026-09-direct-forward-path.md) | Sep 2026 · **NO-GO** · follows #415, #416 | Moved Mode A's writes into the completion handler so the task wakes once per forward, not per buffer. Correct (proxy gate 0/60) and ~3% *slower*, with instructions/byte unchanged at 1.24 — the scheduler round-trip was not the cost. Redirects to gathering, the one remaining difference from `run_direct_echo` |
