...
mod tests {
    ...
    /// bid would be copied to owned at move time instead. That reasoning was
    /// about the close path, and it does not carry: a held buffer is
    /// unconsumed data, which means the quiescent point the handler offered
    /// at has passed. Parking then would ship state deposited before a
    /// request that is already in flight.
    ///
    /// Nothing becomes permanently unparkable, because the handler re-offers
    /// at its next idle point — park is best-effort and simply happens later.
    #[test]
